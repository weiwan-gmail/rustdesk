...
function isIpTarget(id: string): boolean {
  ...
  // bare IPv6 (no port; the colons are part of the address)
...
export default class Connection {
  ...
  async start(id: string) {
    try {
      ...
      // Server-mode Caddy/localserver have no /direct proxy.
  ...
  async _startDirect(target: string) {
    if (!this._options) {
      this._options = globals.getPeers()[target] || {};
    }
    if (!this._password) {
...
function getrUriFromRs(
  ...
  const wsPath = isRelay
    ? CONF.wsRelayPath || "/ws/relay"
    : CONF.wsIdPath || "/ws/id";
  if (!uri) {
