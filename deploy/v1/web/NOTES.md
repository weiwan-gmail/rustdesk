...
1. **`check_ws()` 语义**（`libs/hbb_common/src/websocket.rs`）：域名 → `ws(s)://domain/ws/id|/ws/relay`（路径式，走 80/443 反代）；IP → `ws://IP:21118|21119`（端口偏移 +2/+3）。vendor 树里 `connection.ts` 的 `getrUriFromRs` 完全对齐这套规则，并加了「空服务器 → 同源 `location.host/ws/*`」（配合内置代理实现零配置）。
...
6. **直连开关**：`_startDirect` 在 vendor 树里始终存在，但 `start()` 只在 `RUSTDESK_CONFIG.direct` 为真时走 IP→`/direct`。服务器模式不设该标志，避免打到 Caddy/localserver 没有的 `/direct`。
