...
与 `deploy/web`（服务器模式）完全独立：不同 exe、不同目录、不同补丁，可同机不同端口并存。
...
- Web 客户端在 `connection.ts` 的 `start()` 里识别 IP 走 `_startDirect`（`patches/0002-direct-connect.patch`，叠加在 `deploy/web` 的 `0001` 之上）。
...
- 其余与 `deploy/web` 相同（v1.2.4 时代 UI、音频依赖重建的 libopus.js）。
