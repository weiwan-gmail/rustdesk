#!/usr/bin/env bash
# Build the rustdesk-web-v2-direct binary (v2 direct-mode web client embedded).
#
#   ./build.sh            # binary for the current platform
#   ./build.sh --all      # linux/amd64, linux/arm64, windows/amd64, darwin/*
#
# Builds the v2 client from the repository's own flutter/ tree (Flutter
# 3.24.5). Direct mode is enabled by writing `direct: true` into the embedded
# config.js (server-mode deploy/v2/web omits it).
set -euo pipefail
...
# flutter build web copies all of web/ into the output; drop build-time-only
# files (node_modules, TS sources, codegen scripts), keep js/dist.
echo ">> staging static assets"
rm -rf "$STATIC"
mkdir -p "$STATIC"
cp -r "$SRC/build/web/." "$STATIC/"
rm -rf "$STATIC/js"
mkdir -p "$STATIC/js"
cp -r "$WEB/js/dist" "$STATIC/js/dist"
cp "$HERE/config.js" "$STATIC/config.js"
# Keep staged assets untracked (build output), except the placeholder pair.
...
build_bin() { # <goos> <goarch> <suffix>
  local out="$HERE/rustdesk-web-v2-direct$3"
...
echo ">> done: $HERE/rustdesk-web-v2-direct"
