...
- `js/package.json`：固定 `libsodium`/`libsodium-wrappers@0.7.13`、`typescript@6.0.3`、`@types/node@26.3.0`（npm `overrides` 钉死；0.7.16+ 的 ESM 会让 vite 2.8 找不到 `./libsodium.mjs`）。`tsconfig.json` 使用 `moduleResolution: "bundler"`、`skipLibCheck`、`stableTypeOrdering`、`types: ["node"]`（给 TS 7 铺路；本栈不装 7.0，因其尚无 Compiler API）。`vite.config.js` 把 sodium 别名到 CJS 构建。`pin-js-deps.sh` 在安装前再写一遍这些钉死版本。
