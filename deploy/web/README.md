...
- `js/package.json`：固定 `libsodium`/`libsodium-wrappers@0.7.13`（新版 ESM 布局与 vite 2.8 不兼容）与 `@types/node@16.18.68`（直接依赖 + 无 glob 的 `resolutions`；Yarn 1 会忽略 `**/@types/node`，新版类型语法超出 TS 4.4 解析能力）。构建脚本在 `yarn install` 前还会再跑一遍 `pin-js-deps.sh`。
